  ...
  try {
    ...
    if (cards.length > 0) {
      for (let i = 0; i < Math.min(cards.length, 2); i++) {
        ...
        console.log(
          `    Animation detected: ${hasTransformChange || hasOpacityChange || hasShadowChange ? '✅' : '❌'}`
        );
    ...
    console.log(
      `  Elements with animations in reduced motion: ${elementsWithAnimations.length}`
    );
    if (elementsWithAnimations.length > 0) {
      console.log(
        '  ⚠️ Some animations may still be active in reduced motion mode'
      );
      elementsWithAnimations.slice(0, 3).forEach((el, i) => {
        console.log(
          `    ${i + 1}. ${el.tagName}.${el.className}: ${el.animationName}, ${el.transitionDuration}`
        );
      });
    } else {
      console.log(
        '  ✅ All animations properly disabled in reduced motion mode'
      );
    }
    ...
    console.log(
      `  ${interactionTime < 500 ? '✅' : '⚠️'} Performance ${interactionTime < 500 ? 'excellent' : 'needs attention'}`
    );
    ...
    for (const viewport of viewports) {
      ...
      if (button) {
        ...
        console.log(
          `    Transition: ${responsiveStyles.transition}, Transform: ${responsiveStyles.transform !== 'none' ? 'Active' : 'None'}`
        );
      }
    }
    ...
    const pageAnimations = await page.evaluate(() => {
      ...
      const animatedElements = allElements.filter(el => {
        const styles = getComputedStyle(el);
        return (
          styles.transitionDuration !== '0s' || styles.animationName !== 'none'
        );
      });
      ...
      return {
        ...
        percentage: (
          (animatedElements.length / allElements.length) *
          100
        ).toFixed(1),
      };
    });
    ...
    console.log(
      `  Elements with animations: ${pageAnimations.animatedElements}`
    );
    ...
    const nebulaCheck = await page.evaluate(() => {
      ...
      const avgButtonDuration = buttonTransitions.length
        ? buttonTransitions.reduce((a, b) => a + b, 0) /
          buttonTransitions.length
        : 0;
      const avgCardDuration = cardTransitions.length
        ? cardTransitions.reduce((a, b) => a + b, 0) / cardTransitions.length
        : 0;
    ...
    console.log(
      `  Average button transition: ${nebulaCheck.avgButtonDuration}ms`
    );
    console.log(`  Average card transition: ${nebulaCheck.avgCardDuration}ms`);
    console.log(
      `  Nebula "minimal and snappy" criteria: ${nebulaCheck.fastAnimations ? '✅ Met' : '❌ Not met'}`
    );
    ...
    const overallScore = Object.values(evaluation).filter(
      v => v === 'EXCELLENT'
    ).length;
    console.log(
    ...
    if (overallScore >= 4) {
      ...
      console.log(
        '✅ Nebula UI animation refinements are successful with minor improvements needed'
      );
    } else {
