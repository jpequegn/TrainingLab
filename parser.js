  ...
  for (const segment of segments) {
    ...
    if (segmentData) {
      if (Array.isArray(segmentData)) {
        ...
        const totalIntervalDuration = segmentData.reduce(
...
function parseSegment(segment, startTime) {
  ...
  switch (tagName) {
    case 'Warmup':
    case 'Cooldown':
      segmentData.powerLow =
        parseFloat(segment.getAttribute('PowerLow')) || 0.5;
      segmentData.powerHigh =
        parseFloat(segment.getAttribute('PowerHigh')) || 0.7;
    ...
    case 'Ramp':
      segmentData.powerLow =
        parseFloat(segment.getAttribute('PowerLow')) || 0.5;
      segmentData.powerHigh =
        parseFloat(segment.getAttribute('PowerHigh')) || 1.0;
      segmentData.powerData = generateRampData(segmentData);
      break;
