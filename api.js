...
export async function fetchDirectory(path = '') {
    try {
        ...
        if (!response.ok) {
            console.warn(`Failed to fetch directory ${path}: ${response.status} ${response.statusText}`);
            return [];
        }
...
export async function fetchWorkoutFile(filePath) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(`Failed to fetch workout file (${response.status}): ${errorText}`);
        }
...
export async function getZwiftWorkoutDirectory() {
    try {
        ...
        if (!response.ok) {
            console.warn(`Failed to get Zwift directory: ${response.status} ${response.statusText}`);
            return null;
        }
...
export async function selectFolder() {
    try {
        ...
        if (!response.ok) {
            console.warn(`Failed to select folder: ${response.status} ${response.statusText}`);
            return null;
        }
...
export async function getMCPStatus() {
    try {
        ...
        if (!response.ok) {
            const errorData = await response.json().catch(() => ({}));
            throw new Error(errorData.error || `Failed to get MCP status (${response.status})`);
        }
...
export async function controlMCPServer(action, serverId) {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(result.error || `Failed to ${action} server (${response.status})`);
        }
