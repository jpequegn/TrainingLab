export function calculateTSS(workout) {
  ...
  // Calculate TSS
  const tss =
    ((totalDuration * normalizedPower * intensityFactor) / (1.0 * 3600)) * 100;
...
export function calculatePowerCurve(
  workout,
  durations = [5, 10, 15, 20, 30, 60, 300, 600, 1200, 3600]
) {
  if (!workout.segments || workout.segments.length === 0) {
    return {};
  }
...
export function calculateWorkoutDifficultyScore(workout, metrics) {
  ...
  if (duration <= 0.5)
    durationComponent = duration * 8; // Up to 4 points for 30min
  else if (duration <= 1.0)
    durationComponent = 4 + (duration - 0.5) * 8; // 4-8 points for 30-60min
  else if (duration <= 2.0)
    durationComponent = 8 + (duration - 1.0) * 8; // 8-16 points for 1-2h
  ...
  if (timeInZones) {
    ...
    intensityComponent =
      (zone4Time * 0.4 + zone5Time * 0.8 + zone6Time * 1.2 + zone7Time * 1.6) /
...
export class Workout {
  ...
  scaleSegmentPower(segment, scaleFactor) {
    ...
    if (segment.powerLow !== undefined) {
      segment.powerLow = parseFloat(
        (segment.powerLow * scaleFactor).toFixed(2)
      );
    }
    if (segment.powerHigh !== undefined) {
      segment.powerHigh = parseFloat(
        (segment.powerHigh * scaleFactor).toFixed(2)
      );
    }
  }
  ...
  updateSegmentPowerData(segment) {
    if (
      segment.type.includes('Interval') ||
      segment.type === 'SteadyState' ||
      segment.type === 'FreeRide'
    ) {
