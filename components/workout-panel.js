...
export class WorkoutPanel extends BaseComponent {
  ...
  createPanelStructure() {
    const layoutClass =
      this.options.layout === 'horizontal' ? 'flex-row' : 'flex-col';
  ...
  async initializeChildComponents() {
    ...
    if (this.options.showChart) {
      const chartElement = this.element.querySelector(
        '[data-component-type="workout-chart"]'
      );
      if (chartElement) {
    ...
    if (this.options.showSegmentEditor) {
      const editorElement = this.element.querySelector(
        '[data-component-type="segment-editor"]'
      );
      if (editorElement) {
