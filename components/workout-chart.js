...
export class WorkoutChart extends BaseComponent {
    ...
    setupStateBindings() {
        ...
        this.subscribe('themeMode', (theme) => {
    ...
    async addPowerZones() {
        try {
            const chartAnnotations = await moduleLoader.loadModule('chart-annotations');
    ...
    highlightSegment(segmentIndex) {
        ...
        if (this.chartInstance.options.plugins.annotation?.annotations?.highlight) {
            delete this.chartInstance.options.plugins.annotation.annotations.highlight;
        }
