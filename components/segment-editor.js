...
export class SegmentEditor extends BaseComponent {
    ...
    loadSegment(segmentIndex) {
        ...
        // Update segment info
        const segmentInfo = this.element.querySelector('[data-bind="segment-index"]');
        if (segmentInfo) {
    ...
    updateFieldVisibility(segment) {
        ...
        const hasRange = segment.powerLow !== undefined || segment.powerHigh !== undefined;
        
        const powerLowField = this.element.querySelector('[data-field="powerLow"]');
        const powerHighField = this.element.querySelector('[data-field="powerHigh"]');
    ...
    validateForm(data) {
        ...
        if (data.powerLow !== null && data.powerHigh !== null) {
            if (data.powerLow >= data.powerHigh) {
                this.setFieldError('powerHigh', 'High power must be greater than low power');
                isValid = false;
            }
        }
    ...
    setEditable(editable) {
        ...
        inputs.forEach(input => {
            if (input.type === 'submit') {
                input.disabled = !editable || this.validationErrors.size > 0 || !this.hasChanges();
            } else {
