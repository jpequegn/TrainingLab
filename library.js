...
export class WorkoutLibrary {
  ...
  createLibraryInterface() {
    ...
    libraryPanel.className =
      'fixed left-0 top-0 h-full w-80 bg-white/95 backdrop-blur-sm shadow-2xl border-r border-white/20 z-30 transform -translate-x-full transition-transform duration-300';
  ...
  addLibraryToggleButton() {
    ...
    libraryButton.className =
      'inline-flex items-center px-4 py-2 bg-white/10 hover:bg-white/20 text-white rounded-lg transition-colors duration-200 text-sm font-medium backdrop-blur-sm border border-white/20';
  ...
  bindLibraryEvents() {
    ...
    document
      .getElementById('libraryToggleBtn')
    ...
    document
      .getElementById('toggleLibraryPanel')
    ...
    // View switching
    document
      .getElementById('viewAllWorkouts')
      ?.addEventListener('click', () => {
        this.switchView('library');
      });

    document
      .getElementById('viewCollections')
      ?.addEventListener('click', () => {
    ...
    // Actions
    document
      .getElementById('addCurrentWorkout')
      ?.addEventListener('click', () => {
  ...
  switchView(view) {
    ...
    document.querySelectorAll('[data-view]').forEach(btn => {
      if (btn.dataset.view === view) {
        btn.className =
          'flex-1 py-2 px-3 rounded-md text-sm font-medium transition-all duration-200 bg-white text-gray-900 shadow-sm';
      } else {
        btn.className =
          'flex-1 py-2 px-3 rounded-md text-sm font-medium transition-all duration-200 text-gray-600 hover:text-gray-900';
      }
    });
  ...
  async refreshLibraryView() {
    ...
    try {
      ...
      content.innerHTML =
        '<div class="text-center text-gray-500 py-8">Error loading library</div>';
    }
  }
  ...
  async renderCollectionView() {
    ...
    try {
      ...
      content.innerHTML =
        '<div class="text-center text-gray-500 py-8">Error loading collections</div>';
    }
  }
