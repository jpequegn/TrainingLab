...
export class ThemeManager {
  ...
  applyLightTheme() {
    ...
    root.style.setProperty(
      '--bg-primary',
      'linear-gradient(to bottom right, #E0E6F0, #A3A9BF, #1F4E8C)'
    );
    root.style.setProperty('--bg-card', 'rgba(224, 230, 240, 0.95)');
    root.style.setProperty(
      '--bg-card-header',
      'linear-gradient(to right, #1F4E8C, #333A56)'
    );
  ...
  applyDarkTheme() {
    ...
    root.style.setProperty(
      '--bg-primary',
      'linear-gradient(to bottom right, #121317, #1E2130, #1F4E8C)'
    );
  ...
  createThemeToggle() {
    ...
    toggleButton.className =
      'w-12 h-12 bg-white/10 hover:bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center transition-all duration-200 border border-white/20';
