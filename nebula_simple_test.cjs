...
async function testNebulaAnimations() {
  ...
  try {
      ...
      return {
        ...
        avgTransitionDuration:
          transitionElements > 0
            ? ((totalTransitionDuration / transitionElements) * 1000).toFixed(0)
            : 0,
        fastTransitions,
        slowTransitions,
        fastTransitionPercentage:
          transitionElements > 0
            ? ((fastTransitions / transitionElements) * 100).toFixed(1)
            : 0,
      };
    });
    ...
    console.log(
      `    Elements with transitions: ${animationAnalysis.transitionElements}`
    );
    console.log(
      `    Elements with animations: ${animationAnalysis.animationElements}`
    );
    console.log(
      `    Average transition duration: ${animationAnalysis.avgTransitionDuration}ms`
    );
    console.log(
      `    Fast transitions (≤200ms): ${animationAnalysis.fastTransitions}`
    );
    console.log(
      `    Slow transitions (>200ms): ${animationAnalysis.slowTransitions}`
    );
    console.log(
      `    Fast transition percentage: ${animationAnalysis.fastTransitionPercentage}%`
    );
    ...
    if (buttons.length > 0) {
      ...
      console.log(
        `    ${avgResponseTime <= 200 ? '✅' : '⚠️'} Response time ${avgResponseTime <= 200 ? 'excellent' : 'needs improvement'}`
      );
    }
    ...
    console.log(
      `    Elements still animated: ${reducedMotionAnalysis.stillAnimated}`
    );
    console.log(
      `    Reduced motion compliance: ${reducedMotionAnalysis.compliance}`
    );
    console.log(
      `    ${reducedMotionAnalysis.compliance === 'EXCELLENT' ? '✅' : '⚠️'} Accessibility ${reducedMotionAnalysis.compliance.toLowerCase()}`
    );
      ...
      return {
        ...
        avgOpacity:
          loadingStates.length > 0
            ? (
                loadingStates.reduce((sum, s) => sum + s.opacity, 0) /
    ...
    console.log(
      `    Loading elements found: ${loadingAnalysis.loadingElementsFound}`
    );
    console.log(
      `    Visible loading states: ${loadingAnalysis.visibleLoaders}`
    );
    if (loadingAnalysis.avgOpacity > 0) {
      console.log(`    Average loading opacity: ${loadingAnalysis.avgOpacity}`);
      const nebulaCompliant =
        parseFloat(loadingAnalysis.avgOpacity) >= 0.8 &&
        parseFloat(loadingAnalysis.avgOpacity) <= 0.9;
      console.log(
        `    ${nebulaCompliant ? '✅' : '⚠️'} Nebula opacity refinement (0.85) ${nebulaCompliant ? 'detected' : 'not detected'}`
      );
    }
    ...
    console.log(
      `    Average interaction time: ${perfResults.avgInteractionTime}ms`
    );
    console.log(
      `    ${perfResults.avgInteractionTime <= 100 ? '✅' : '⚠️'} Performance ${perfResults.avgInteractionTime <= 100 ? 'excellent' : 'good'}`
    );
    ...
    for (const viewport of viewports) {
      ...
      const responsiveCheck = await page.evaluate(() => {
        ...
        return {
          buttonsVisible: Array.from(buttons).filter(
            b => b.offsetParent !== null
          ).length,
          cardsVisible: Array.from(cards).filter(c => c.offsetParent !== null)
            .length,
        };
      });

      console.log(
        `    ${viewport.name}: ${responsiveCheck.buttonsVisible} buttons, ${responsiveCheck.cardsVisible} cards visible`
      );
    }
    ...
    const nebulaScore = {
      timing:
        parseFloat(animationAnalysis.avgTransitionDuration) <= 200
          ? 'EXCELLENT'
          : 'GOOD',
      fastPercentage:
        parseFloat(animationAnalysis.fastTransitionPercentage) >= 80
          ? 'EXCELLENT'
          : 'GOOD',
      accessibility: reducedMotionAnalysis.compliance,
      performance:
        parseFloat(perfResults.avgInteractionTime) <= 100
          ? 'EXCELLENT'
          : 'GOOD',
    ...
    console.log(
      `  ⚡ Animation Timing (avg ${animationAnalysis.avgTransitionDuration}ms): ${nebulaScore.timing}`
    );
    console.log(
      `  🎯 Fast Animations (${animationAnalysis.fastTransitionPercentage}%): ${nebulaScore.fastPercentage}`
    );
    ...
    const excellentCount = Object.values(nebulaScore).filter(
      v => v === 'EXCELLENT'
    ).length;
    ...
    if (excellentCount >= 4) {
      console.log(
        '✅ OUTSTANDING: Nebula UI animation refinements are highly successful!'
      );
      console.log(
        '   The animations are minimal, snappy, and provide subtle feedback without distraction.'
      );
    } else if (excellentCount >= 3) {
      console.log(
        '✅ SUCCESSFUL: Nebula UI animation refinements are working well with minor room for improvement.'
      );
    } else {
      console.log(
        '⚠️ IMPROVEMENT NEEDED: Nebula UI animation refinements need additional optimization.'
      );
    }
    ...
    if (parseFloat(animationAnalysis.avgTransitionDuration) > 200) {
      console.log(
        '  • Consider reducing transition durations to ≤200ms for snappier feel'
      );
    }
    ...
    if (reducedMotionAnalysis.compliance !== 'EXCELLENT') {
      console.log(
        '  • Improve reduced motion compliance for better accessibility'
      );
    }
    if (parseFloat(perfResults.avgInteractionTime) > 100) {
      console.log(
        '  • Optimize interaction response times for better performance'
      );
    }
  } catch (error) {
