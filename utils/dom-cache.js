...
export class DOMCache {
  ...
  getAll(selector, context = document) {
    ...
    if (this.cache.has(key)) {
      ...
      if (cached.elements && cached.elements.every(el => this.isElementValid(el))) {
  ...
  batch(operations) {
    ...
    operations.forEach(op => {
      try {
        ...
        logger.error('Batch operation failed', error, { operation: op.toString() });
        results.push(null);
      }
    });
  ...
  revalidateCache() {
    ...
    this.cache.forEach((cached, key) => {
      ...
      } else if (cached.elements && !cached.elements.every(el => this.isElementValid(el))) {
        invalidKeys.push(key);
      }
    });
    ...
    if (invalidKeys.length > 0) {
      logger.debug('Cache revalidated', { invalidatedEntries: invalidKeys.length });
    }
  }
