...
export class WorkoutEditor {
    ...
    setupEventListeners() {
        ...
        if (closeSegmentEditor) {
            closeSegmentEditor.addEventListener('click', () => this.closeSegmentEditor());
        }
    ...
    updateTimeline() {
        ...
        timelineSegments.innerHTML = this.segments.map((segment, index) => {
    ...
    updateWorkoutSummary() {
        ...
        // Update display
        document.getElementById('editorTotalDuration').textContent = formatDuration(totalDuration);
        document.getElementById('editorAvgPower').textContent = `${Math.round(avgPower * 100)}%`;
        document.getElementById('editorEstTSS').textContent = Math.round(estimatedTSS);
    }
    ...
    saveSegmentChanges(segment) {
        ...
        const duration = parseFloat(document.getElementById('segmentDuration')?.value) * 60;
    ...
    segmentsToWorkoutData() {
        ...
        const segments = this.segments.map(segment => {
            ...
            segmentData.powerData = this.generateSegmentPowerData(segment, currentTime);
