...
export class KeyboardNavigation {
  ...
  isTypingContext(element) {
    ...
    return (
      typingElements.includes(element.tagName.toLowerCase()) ||
    ...
    if (this.modalStack.length > 0) {
      ...
      if (modal && modal.parentNode) {
        const closeBtn = modal.querySelector(
          '[data-dismiss="modal"], .close-btn, button[onclick*="remove"]'
        );
        if (closeBtn) {
          closeBtn.click();
        } else {
  ...
  handleEnter() {
    ...
    if (focused && focused.tagName === 'BUTTON') {
      ...
      this.chartNavigation &&
      this.chartNavigation.selectedSegment !== null
    ) {
...
class ChartNavigation {
  ...
  setupChartKeyboardSupport() {
    ...
    this.canvas.setAttribute(
      'aria-label',
      'Workout power profile chart. Use arrow keys to navigate segments.'
    );
  }
