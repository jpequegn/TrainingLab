...
test.describe('Issue #56 UI Bug Fixes', () => {
  ...
  test('LLM sidebar should be visible on larger screens', async ({ page }) => {
    ...
    const transform = await chatPanel.evaluate(
  ...
  test('LLM sidebar should be hidden on smaller screens initially', async ({
    page,
  }) => {
  ...
  test('Sample workout button should be clickable and show loading', async ({
    page,
  }) => {
    ...
    await expect(
      page.locator('#workoutSection, .loading-overlay, .toast')
  ...
  test('TSS should be calculated and displayed', async ({ page }) => {
    ...
    if (await workoutStats.isVisible()) {
      ...
      const tssElements = page
        .locator('[data-bind="workout-tss"], .workout-tss, text="TSS"')
        .first();
  ...
  test('UI should be responsive and layout correctly', async ({ page }) => {
    ...
    for (const viewport of viewports) {
      ...
      await expect(
        page.locator('nav, header, .container, main').first()
      ).toBeVisible();
    }
  });
