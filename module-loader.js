...
export class ModuleLoader {
  ...
  async createLoadingPromise(moduleName, options) {
    ...
    for (let attempt = 0; attempt < retries; attempt++) {
      try {
        ...
        lastError = error;
        console.warn(
          `Module load attempt ${attempt + 1} failed for ${moduleName}:`,
          error
        );
  ...
  async loadModules(moduleList) {
    ...
    const normalPriority = prioritized.filter(
      m => m.priority === 'normal' || !m.priority
    );
    ...
    for (const moduleConfig of normalPriority) {
      try {
        results[moduleConfig.name] = await this.loadModule(
          moduleConfig.name,
          moduleConfig
        );
      } catch (error) {
    ...
    scheduleLoad(async () => {
      for (const moduleConfig of modules) {
        try {
          ...
          console.warn(
            `Failed to load low priority module: ${moduleConfig.name}`,
            error
          );
        }
      }
