...
export function generateERGContent(workoutData, ftp) {
    ...
    const courseData = dataPoints.map(point => 
        `${(point.time / 60).toFixed(2)}\t${Math.round(point.power)}`
    ).join('\n');
...
function generateWorkoutDataPoints(workoutData, format, ftp = 250) {
    ...
    allSegments.forEach(segment => {
        ...
        if (currentTime < startTime) {
            points.push({ time: startTime, power: format === 'watts' ? 0.5 * ftp : 50 }); // Easy spinning
        }

        if (segment.type === 'Warmup' || segment.type === 'Cooldown' || segment.type === 'Ramp') {
            // Add start and end points for ramps
            points.push({ time: startTime, power: format === 'watts' ? segment.powerLow * ftp : segment.powerLow * 100 });
            points.push({ time: endTime, power: format === 'watts' ? segment.powerHigh * ftp : segment.powerHigh * 100 });
        } else {
            // Steady state or intervals
            const power = segment.power || 0.6;
            points.push({ time: startTime, power: format === 'watts' ? power * ftp : power * 100 });
            points.push({ time: endTime, power: format === 'watts' ? power * ftp : power * 100 });
        }
